...
test('detects errors with Organisation when startYear != endYear', (done) => {
  const result = runProcess("./tests/test-set", 2012, 2019, "Organisation", false).then((result) => {
    ...
    expect(consistencise(err)).toBe("Some copyright headers are wrong  😱 tests/test-set/correct_year-with_org.js")
    done()
  })
})
...
test('detects errors with Organisation when startYear = endYear', (done) => {
  const result = runProcess("./tests/test-set", 2019, 2019, "Organisation", false).then((result) => {
    ...
    expect(consistencise(err)).toBe("Some copyright headers are wrong  😱 tests/test-set/correct_year_range-with_org.js tests/test-set/wrong_year_range-with_org.txt")
    done()
  })
})
